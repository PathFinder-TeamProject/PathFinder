...
public class GatewayConfig {
    ...
    public RouteLocator customRouteLocator(RouteLocatorBuilder builder) {
        return builder.routes()
                ...
                .route("delivery-manager-service", r -> r.path("/v1/delivery-managers/**")
                        .filters(f -> f.filter(jwtAuthenticationFilter.apply(new JwtAuthenticationFilter.Config())))
                        .uri("lb://DELIVERY-MANAGER-SERVICE"))

                // Hub Service
                .route("hub-service", r -> r.path("/v1/hubs/**")
                        .filters(f -> f.filter(jwtAuthenticationFilter.apply(new JwtAuthenticationFilter.Config())))
                        .uri("lb://HUB-SERVICE"))

                // Company Service
                .route("company-service", r -> r.path("/v1/companys/**")
                        .filters(f -> f.filter(jwtAuthenticationFilter.apply(new JwtAuthenticationFilter.Config())))
                        .uri("lb://COMPANY-SERVICE"))

                // Delivery Service
                .route("delivery-service", r -> r.path("/v1/deliverys/**")
                        .filters(f -> f.filter(jwtAuthenticationFilter.apply(new JwtAuthenticationFilter.Config())))
                        .uri("lb://DELIVERY-SERVICE"))

                // Product Service
                .route("product-service", r -> r.path("/v1/products/**")
                        .filters(f -> f.filter(jwtAuthenticationFilter.apply(new JwtAuthenticationFilter.Config())))
                        .uri("lb://PRODUCT-SERVICE"))

                // Messages Service
                .route("messages-service", r -> r.path("/v1/slack-messages/**")
                        .filters(f -> f.filter(jwtAuthenticationFilter.apply(new JwtAuthenticationFilter.Config())))
